...
def main():
    ...
    text_encoder_t5 = T5EncoderModel.from_pretrained("//zoo/google-t5/t5-large",)
    ...
    class MixedIterableDataset(torch.utils.data.IterableDataset):
        def __init__(self, prob = [1.0, 0.0, 0.0]):
            self.probs = prob
        ...
        def __iter__(self):
            ...
            jdb_done=coco_done=lcs_done=False

            while not (jdb_done and coco_done and lcs_done):
    ...
    train_dataset = MixedIterableDataset([1.0, 0.0, 0.0])
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        while step < args.max_train_steps:
            ...
            with torch.no_grad():
                ...
                image.save(f"{args.save_dir}/{save_base}.png")
                toc = time.time()
                logs = {
                    "time": toc-tic,
                }
