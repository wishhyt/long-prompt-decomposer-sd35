...
class TextDecomposer(nn.Module):
    def __init__(
        ...
        # ELLA
        self.mask_head = PromptDecomposer(
            width=width,
            heads=heads,
            layers=layers,
    ...
    def forward(self, encoder_hidden_states, timesteps):
        bs, seq_len, hidden_dim = encoder_hidden_states.shape
        encoder_hidden_state_list = self.mask_head(encoder_hidden_states, timesteps=timesteps)
...
def main():
    ...
    if args.tune_unet:
        if args.lora_target_modules is not None:
            ...
            lora = "//logs/long_PDD/sd1/full/1_component_T256_ELLA_init-768-8-6_D10_tuneU_3467/checkpoint-8000/pytorch_lora_weights.safetensors"
            print('loaded ', lora)
            sd = load_file(lora)
            refined_sd = dict()
            for k,v in sd.items():
            ...
            mk, uk = unet.load_state_dict(refined_sd, strict=False)
    ...
    # train_dataset_jdb = train_dataset_jdb.rename_column("pixel_values", "jpg").rename_column("url", "__key__").remove_columns('captions')
    # train_dataset_coco = train_dataset_coco.remove_columns('__url__')
    # train_dataset_lcs = train_dataset_lcs.remove_columns('__url__')
    # train_dataset_sam = train_dataset_sam.rename_column("pixel_values", "jpg")
    ...
    #     seed=args.seed,
    # )
    train_dataset = train_dataset_jdb
