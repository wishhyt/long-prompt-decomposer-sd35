...
from lpd.modules.lora import inject_trainable_lora_extended, save_lora_weight
...
from lpd.tools import load_dataset, collate_fn, caption2embed, sample_images
from lpd.tools import get_predicted_original, DDIMSolver
...
def main(args):
    ...
    vis, adapter, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        vis, adapter, optimizer, train_dataloader, lr_scheduler,)
        ...
        for _, batch in enumerate(train_dataloader):
            with accelerator.accumulate(vis, adapter):
                with torch.no_grad():
                    ...
                    start_timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (latents.shape[0],),
