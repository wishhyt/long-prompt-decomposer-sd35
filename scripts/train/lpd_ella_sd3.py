...
def log_validation(decomposer, caption2embed_simple, transformer, vae, text_encoder, text_encoder_2, text_encoder_3, tokenizer, tokenizer_2, tokenizer_3, args, accelerator, weight_dtype, epoch, return_mask=False):
    ...
    with autocast_ctx:
        image = pipeline(
            ...
            encoder_hidden_states_clip_pooled,
            prompt="",
...
class TextDecomposer(torch.nn.Module):
    def __init__(
        ...
        # ELLA
        self.mask_head = PromptResampler_ct5(
            width=width,
            heads=heads,
            layers=layers,
    ...
    def forward(self, encoder_hidden_state_t5, encoder_hidden_state_clip):
        bs, seq_len, hidden_dim = encoder_hidden_state_t5.shape
        encoder_hidden_state_list = self.mask_head(encoder_hidden_state_t5, encoder_hidden_state_clip)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(decomposer):
                ...
                model_pred_batch = transformer(
                    ...
                    encoder_hidden_states=decomposed_encoder_hidden_states,
                    pooled_projections=torch.cat([encoder_hidden_states_clip_pooled]*args.num_components),
                    return_dict=False,
                )[0]
