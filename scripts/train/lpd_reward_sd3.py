...
def parse_args(input_args=None):
    ...
    parser.add_argument("--max_grad_norm", default=0.3, type=float, help="Max gradient norm.")
...
def main(args):
    ...
    # timesteps_sde = scheduler.timesteps

    # Determine which steps to backpropagate through to save memory
    time_all = scheduler.timesteps
    sigmas = scheduler.sigmas
    rand_start = random.randint(0, 9) # Adjust for smaller step count
    time_bp = time_all[rand_start::10].tolist() # e.g., backprop every 10th step
    ...
    # Optimizer creation
    params_to_optimize = [p for n, p in transformer.named_parameters() if p.requires_grad]
    ...
    # prepare negative conditions
    with torch.no_grad():
        encoder_hidden_states_neg = caption2embed_simple([''])
        # encoder_hidden_states_clip = encoder_hidden_states_pre['encoder_hidden_states_clip_concat']
        # encoder_hidden_states_clip_pooled = encoder_hidden_states_pre['encoder_hidden_states_clip_pooled']
        t5_hidden_states_neg = encoder_hidden_states_neg["encoder_hidden_states_t5"]
        clip_prompt_embeds_neg, pooled_prompt_embeds_neg = clip_encode_prompt(
            text_encoders,
            tokenizers,
            [''],
            args.token_length,
        ...
        token_hidden_states_neg = decomposer(t5_hidden_states_neg)
        decomposed_hidden_states_neg = torch.cat(token_hidden_states_neg).to(dtype=weight_dtype)
        exp_clip_prompt_embeds_neg = torch.cat([clip_prompt_embeds_neg]*args.num_components)
        prompt_embeds_neg = torch.cat([exp_clip_prompt_embeds_neg, decomposed_hidden_states_neg], dim=-2)
        prompt_embeds_neg = prompt_embeds_neg.repeat_interleave(args.train_batch_size, dim=0)
        pooled_prompt_embeds_neg = torch.cat([pooled_prompt_embeds_neg]*args.train_batch_size*args.num_components)
    ...
    image_logs = None
    ortho_weight = 1. - 0.3
    shape = (args.train_batch_size, 16, args.resolution // 8, args.resolution // 8)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                with torch.no_grad():
                    ...
                    noise = torch.randn(shape, device=accelerator.device, dtype=weight_dtype)
                ...
                for i, t in enumerate(time_all):
                    ...
                    dt = sigma_next - sigma
                    # Expand the latents to match the batch size for CFG and components
                    # Total batch size = B * num_components * 2 (for CFG)
                    latent_model_input = x_prev.repeat(args.num_components, 1, 1, 1)
                    timestep = t.expand(latent_model_input.shape[0])

                    # 4. Predict the model output (velocity)
                    # Use checkpointing for memory efficiency on specified steps
                    if t.item() in time_bp:
                        model_output = checkpoint.checkpoint(
                            transformer,
                            latent_model_input.detach(),
                            prompt_embeds,
                            torch.cat([pooled_prompt_embeds]*args.num_components),
                            timestep,
                            use_reentrant=False
                        ).sample
                        model_output_neg = checkpoint.checkpoint(
                            transformer,
                            latent_model_input.detach(),
                            prompt_embeds_neg,
                    ...
                    # 5. Perform Classifier-Free Guidance and Component Averaging
                    model_output_cfg = model_output_neg + w * (model_output - model_output_neg)

                    # Average over your components
                    # final_model_output = model_output_cfg.view(args.num_components, args.train_batch_size, 16, 64, 64).mean(dim=0)
                    final_model_output = sum(model_output_cfg.chunk(args.num_components)) / args.num_components

                    # 6. Compute the previous noisy sample using the SDE step formula
                    # This replaces `solver.ddim_step`
                    x_prev = x_prev + dt*final_model_output

                # these weighting schemes use a uniform timestep sampling
                # and instead post-weight the loss
                pred_x_0 = vae.decode(x_prev.to(vae.dtype) / vae.config.scaling_factor + vae.config.shift_factor, return_dict=False)[0]
                ...
                if accelerator.is_main_process:
                    if global_step % 10 == 0:
                        ...
                        for tracker in accelerator.trackers:
                            if tracker.name == "wandb":
                                tracker.log(
                                    {
                                        "log": wandb.Image(pil_image, caption=f"{batch['captions'][0]}")
                                    }
                                )
                    ...
                    if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                        images = log_validation(
                            decomposer, caption2embed_simple, unwrap_model(transformer), vae, text_encoder_one, text_encoder_two, text_encoder_three, tokenizer_one, tokenizer_two, tokenizer_three, args, accelerator, weight_dtype, epoch
                        )

            logs = {
                # "loss": kd_loss.detach().item(),
                "diff": loss.detach().item(),
